...
class CurveGrouping(nn.Module):
    ...
    def forward(self, x, xyz, idx):
        ...
        _, start_index = torch.topk(x_att,
                                    ...
                                    dim=2,
                                    sorted=False)
        #print('start_index size: ', start_index.size())
        start_index = start_index.squeeze().unsqueeze(2)
        #start_index = start_index.squeeze(dim=1).unsqueeze(2)
        #print('start_index size after squeeze unsqueeze: ', start_index.size())
