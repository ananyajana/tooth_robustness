...
output_path = '/research/cbim/medical/aj611/experiments/biomed_2022/mesh/data/3dteethseg_all_vtps_combined_flipped_16k_8class_meshes_cropped_front/'
...
s.crop(right=0.30).crop(left=0.35).crop(front=0.5) # this is cur in such a way that sample_01_d.vtp has a single molar after this operation
...
for i in range(len(all_files)):
    ...
    s_mod = s.crop(right=0.30).crop(left=0.35).crop(front=0.5)
    #print('labels len for s_mod:', len(s_mod.celldata['labels']))
    #s.show()
    #s_mod.show()
    #raise ValueError("Exit!")
