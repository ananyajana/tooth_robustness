...
#output_path = '/research/cbim/medical/aj611/experiments/biomed_2022/mesh/data/3dteethseg_all_vtps_combined_flipped_16k_8class_meshes_cropped_molar/'
output_path = '/research/cbim/medical/aj611/experiments/biomed_2022/mesh/data/3dteethseg_all_vtps_combined_flipped_16k_8class_meshes_cropped_four_teeth/'
...
# check the specific file sample_024_d.vtp
#print(all_files)
...
f1 = os.path.join(dataurl, 'Sample_042_d.vtp')
...
#s.crop(right=0.48).crop(bottom=0.15).crop(front=0.16).crop(back=0.7) # this is cur in such a way that sample_01_d.vtp has a single molar after this operation
s.crop(left=0.56).crop(bottom=0.15).crop(front=0.48)
...
for i in range(len(all_files)):
    ...
    #--------------------- current one
    #s_mod = s.crop(right=0.78).crop(top=0.52) # this is cur in such a way that sample_01_d.vtp has a single molar after this operation
    s_mod = s.crop(right=0.48).crop(bottom=0.15).crop(front=0.44).crop(back=0.33)
