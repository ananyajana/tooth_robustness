...
s.crop(right=0.48).crop(bottom=0.15).crop(front=0.26).crop(back=0.7) # this is cur in such a way that sample_01_d.vtp has a single molar after this operation
...
for i in range(len(all_files)):
    ...
    #--------------------- current one
    #s_mod = s.crop(right=0.78).crop(top=0.52) # this is cur in such a way that sample_01_d.vtp has a single molar after this operation
    s_mod = s.crop(right=0.48).crop(bottom=0.15).crop(front=0.26).crop(back=0.7)
